...
locals {
  ...
  nonprod_origins = merge(
    local.dev_origin_host_name == "" ? {} : {
      dev = {
        origin_group_name = "nonprod"
        host_name         = local.dev_origin_host_name
        host_header       = local.dev_origin_host_header
      }
    },
    local.staging_origin_host_name == "" ? {} : {
      staging = {
        origin_group_name = "nonprod"
...
module "frontdoor" {
  ...
  routes = length(local.nonprod_origins) == 0 ? {} : {
    route-all = {
      origin_group_name   = "nonprod"
      origin_names        = keys(local.nonprod_origins)
      patterns_to_match   = ["/*"]
